...
def end_of_month(value: DateLike, cal: str | None = None) -> datetime:
    dt = parse(value)
...
def start_of_quarter(value: DateLike, cal: str | None = None) -> datetime:
    dt = parse(value)
...
def end_of_quarter(value: DateLike, cal: str | None = None) -> datetime:
    dt = parse(value)
...
def start_of_year(value: DateLike, cal: str | None = None) -> datetime:
    dt = parse(value)
    first = datetime(dt.year, 1, 1, tzinfo=timezone.utc)
    return first if is_business_day(first, cal) else next_business_day(first, cal)


def end_of_year(value: DateLike, cal: str | None = None) -> datetime:
    dt = parse(value)
...
def end_of_week(value: DateLike, cal: str | None = None) -> datetime:
